...
contract Briefing {
    ...
    function putSomething(uint256 _numberInput, string memory _nameInput, address _player) public {
        ...
        require(
            keccak256(abi.encodePacked("Casino Heist Player")) == keccak256(abi.encodePacked(_nameInput)),
            "Join the game?"
        );
    ...
    function Finalize(bytes32 _secret) public {
        require(
            completedCall && completedDeposit && completedInputation && completedTransfer,
            "To Finalize, everything must be completed before!"
        );
